...
-- Requires a LOINC reference table with LOINC_NUM and PANEL_TYPE columns.
...
SELECT TABLE_NAME                                                                                  AS "Table",
       FIELD_NAME                                                                                  AS "Field",
       TO_VARCHAR(LOINC_PANEL_RECORDS)                                                             AS "LOINC records with a Panel_Type of 'Panel'",
       TO_VARCHAR(TOTAL_LOINC_RECORDS)                                                             AS "LOINC records",
       TO_VARCHAR(ROUND(LOINC_PANEL_RECORDS * 100.0 / NULLIF(TOTAL_LOINC_RECORDS, 0), 1)) || '%'   AS "% of LOINC records that are panels"
FROM (
