...
SELECT TABLE_NAME AS "TABLE", FIELD_NAME AS "FIELD",
       TO_VARCHAR(NUMERATOR)   AS "Records with future dates__Numerator",
       TO_VARCHAR(DENOMINATOR) AS "Records with future dates__Denominator",
       TO_VARCHAR(ROUND(NUMERATOR * 100.0 / NULLIF(DENOMINATOR, 0), 1)) || '%' AS "Records with future dates__%"
FROM (
    ...
    UNION ALL
    SELECT 'LDS_ADDRESS_HISTORY', 'ADDRESS_PERIOD_END',
           SUM(CASE WHEN ADDRESS_PERIOD_END > h.ANCHOR_DATE THEN 1 ELSE 0 END),
           COUNT(*),
           24
    FROM {{ current_schema }}.LDS_ADDRESS_HISTORY, harvest_anchor h
    WHERE ADDRESS_PERIOD_END IS NOT NULL
    ...
    UNION ALL
    SELECT 'IMMUNIZATION', 'VX_ADMIN_DATE',
           SUM(CASE WHEN VX_ADMIN_DATE > h.ANCHOR_DATE THEN 1 ELSE 0 END),
           COUNT(*),
           26
    FROM {{ current_schema }}.IMMUNIZATION, harvest_anchor h
    WHERE VX_ADMIN_DATE IS NOT NULL

    UNION ALL
    SELECT 'EXTERNAL_MEDS', 'EXT_RECORD_DATE',
           SUM(CASE WHEN EXT_RECORD_DATE > h.ANCHOR_DATE THEN 1 ELSE 0 END),
           COUNT(*),
           27
    FROM {{ current_schema }}.EXTERNAL_MEDS, harvest_anchor h
    WHERE EXT_RECORD_DATE IS NOT NULL

    UNION ALL
    SELECT 'EXTERNAL_MEDS', 'EXT_PAT_START_DATE',
           SUM(CASE WHEN EXT_PAT_START_DATE > h.ANCHOR_DATE THEN 1 ELSE 0 END),
           COUNT(*),
           28
    FROM {{ current_schema }}.EXTERNAL_MEDS, harvest_anchor h
    WHERE EXT_PAT_START_DATE IS NOT NULL

    UNION ALL
    SELECT 'EXTERNAL_MEDS', 'EXT_PAT_END_DATE',
           SUM(CASE WHEN EXT_PAT_END_DATE > h.ANCHOR_DATE THEN 1 ELSE 0 END),
           COUNT(*),
           29
    FROM {{ current_schema }}.EXTERNAL_MEDS, harvest_anchor h
    WHERE EXT_PAT_END_DATE IS NOT NULL

    UNION ALL
    SELECT 'EXTERNAL_MEDS', 'EXT_END_DATE',
           SUM(CASE WHEN EXT_END_DATE > h.ANCHOR_DATE THEN 1 ELSE 0 END),
           COUNT(*),
           30
    FROM {{ current_schema }}.EXTERNAL_MEDS, harvest_anchor h
    WHERE EXT_END_DATE IS NOT NULL

    UNION ALL
    SELECT 'PAT_RELATIONSHIP', 'RELATIONSHIP_START',
           SUM(CASE WHEN RELATIONSHIP_START > h.ANCHOR_DATE THEN 1 ELSE 0 END),
           COUNT(*),
           31
    FROM {{ current_schema }}.PAT_RELATIONSHIP, harvest_anchor h
    WHERE RELATIONSHIP_START IS NOT NULL

    UNION ALL
    SELECT 'PAT_RELATIONSHIP', 'RELATIONSHIP_END',
           SUM(CASE WHEN RELATIONSHIP_END > h.ANCHOR_DATE THEN 1 ELSE 0 END),
           COUNT(*),
           32
    FROM {{ current_schema }}.PAT_RELATIONSHIP, harvest_anchor h
    WHERE RELATIONSHIP_END IS NOT NULL
